...
export default function Home() {
  const someProducts = [
    { src: Bags, title: "Bags" },
    { src: Jeans, title: "Jeans" },
    { src: shamizs, title: "shamizs" },
    { src: blazers, title: "blazers" },
  ];
  return (
    <section className="grow">
      <div className="container mx-auto grid grid-cols-1 gap-x-1 gap-y-5  md:grid-cols-2 px-24 mt-5">
        <HomeSwipr />
        ...
        {someProducts.map((img, index) => (
          <div
            className={`p-5 cursor-pointer  ${
              index % 2 ? "bg-gradient-to-r" : "bg-gradient-to-l"
            }  from-white to-black`}
            key={index}
          >
